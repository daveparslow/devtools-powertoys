...
const CodeEditor = ({ url, lineNumber = 1, columnNumber = 1, functionName = '' }: CodeEditorProps) => {
    const [code, setCode] = useState('');
    const [position, setPosition] = useState({ lineNumber: lineNumber, columnNumber: columnNumber });
    const [formattedPosition, setFormattedPosition] = useState({ lineNumber: 1, columnNumber: 1 });
    ...
    const handleEditorDidMount: OnMount = (editor, monaco) => {
        ...
        monaco.languages.registerDocumentFormattingEditProvider('javascript', {
            provideDocumentFormattingEdits: async function (model: editor.ITextModel) {
                ...
                setPosition({ lineNumber: newPosition.lineNumber, columnNumber: newPosition.column });
    ...
    return (
        <>
            <div className="toolbar">
                <span className='toolbar-item' onClick={goto}><span className='toolbar-label' title={url}>{getFileName(url)} {'>'}</span></span>
                <span className='toolbar-item' onClick={goto}><span className='toolbar-label'>{functionName ? functionName : '(anonymous)'}</span></span>
            </div>
            ...
            <div className="toolbar status-bar">
                <span className='toolbar-item status-bar-item'><span className='toolbar-label' onClick={formatDocument} title='Pretty Print'>{`{ }`}</span></span>
                <span className='toolbar-item status-bar-item' onClick={goto}><span className='toolbar-label'>{`Ln ${position.lineNumber}, Col ${position.columnNumber}, Offset ${offset}`}</span></span>
            </div>
        </>
    );
};
